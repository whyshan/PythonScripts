...
while x < 100: # Loop over x = 1 to 100
	sent = rawinput('gimme a sentence: ') # Ask for a sentence and store it in sent.
	if re.search('[A−Z]{2,5}$',sent): # If there is 2 to 5 times of capital letters (e.g. POS tags) before EOL, do the following.
		...
		sent = re.sub(' [A−Z][a−z]+ [A−Z][a−z]+ [ˆA−Z]', '<PAT2>', sent) # # Replace a space, a capitalized word, a space, another capitalized word, a space and a not capitalized letter with <PAT2>.
